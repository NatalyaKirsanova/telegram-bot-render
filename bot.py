...
class OzonSellerAPI:
    ...
    def get_product_quantity(self, product_info):
        """Получает количество товара в наличии из структуры v3"""
        try:
            stocks_data = product_info.get('stocks', {})
            ...
            if has_stock:
                # Если есть stock но нет конкретного количества, ставим 1
                return 1
...
async def load_real_products():
    ...
    for item in products_data:
        try:
            ...
            print(f"📦 Товар {product_counter}: {name} - {price} ₽ (В наличии: {quantity} шт.)")
            product_counter += 1
